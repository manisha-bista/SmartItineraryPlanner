...
# # Cache-first strategy: DB is checked before every API call.
# # Google is only contacted when we have no cached result.
# # ============================================
...
#       1. Check DB cache (free, no API call).
#       2. Only call Google if cache has no match.
#       3. Save every new result so next search is free.
#     """
...
#     cache_q = cache_q.filter(models.Place.name.ilike(f"%{query}%"))
#     cached = cache_q.limit(5).all()

#     if cached:
#         return {"source": "cache", "results": [_place_to_dict(p) for p in cached]}
...
#                 "region": "np",  # bias results toward Nepal
#             },
#             timeout=10.0
#         )
...
#     # 3. Save new results to DB and return
#     results = []
#     for item in data.get("results", [])[:5]:  # cap at 5 per search
#         place_id = item.get("place_id")
#         if not place_id:
#             continue

#         # race condition guard — skip if already cached
#         existing = db.query(models.Place).filter(models.Place.google_place_id == place_id).first()
#         if existing:
#             results.append(_place_to_dict(existing))
#             continue
...
#     """
#     Redirects to the Google Places photo URL.
#     Photos are fetched on demand only — never pre-fetched.
#     Each call here = one Places Photo API call, so use sparingly.
#     """
#     if not GOOGLE_PLACES_API_KEY:
#         raise HTTPException(status_code=500, detail="Google Places API key not configured")
...
#     """
#     Returns all places already in the DB cache for a given city.
#     Use this to pre-populate dropdowns without any API cost.
#     """
#     q = db.query(models.Place)
#     if city:
#         q = q.filter(models.Place.city.ilike(city))
#     places = q.order_by(models.Place.name).all()
#     return {"results": [_place_to_dict(p) for p in places]}
...
#         "key_preview": GOOGLE_PLACES_API_KEY[:8] + "..." if GOOGLE_PLACES_API_KEY else "missing",
#     }

# # ─────────────────────────────────────────────────────────────────
# # ADD THIS to main.py before the `if __name__ == "__main__":` line
# # ─────────────────────────────────────────────────────────────────
...
#     Expects JSON body with any of: { email, current_password, new_password }
...
#     # ─────────────────────────────────────────────────────────────────
# # ADD THESE TWO ENDPOINTS to main.py before `if __name__ == "__main__":`
# # No email service needed — user proves identity via email lookup,
# # then sets a new password directly. Appropriate for university project.
# # ─────────────────────────────────────────────────────────────────

# @app.post("/auth/forgot-password")
# def forgot_password(payload: dict, db: Session = Depends(get_db)):
#     """
#     Step 1: Verify that the email exists in the system.
#     Returns a token-less confirmation so the frontend can proceed to step 2.
#     We don't reveal whether the email exists to avoid enumeration — but for
#     a university project we return a clear message.
#     """
...
#     """
#     Step 2: Reset password using user_id obtained from step 1.
#     """
...
# # ─────────────────────────────────────────────────────────────────────────────
# # ADD THIS ENDPOINT to main.py — paste before `if __name__ == "__main__":`
# # Also run: pip install google-auth --break-system-packages
# # Also add to .env: GOOGLE_CLIENT_ID=your_client_id_here
# # ─────────────────────────────────────────────────────────────────────────────

# @app.post("/auth/google")
# async def google_auth(payload: dict, db: Session = Depends(get_db)):
#     """
#     Receives Google OAuth access token from frontend,
#     fetches the user profile from Google, then finds or
#     creates the user in our DB. Returns same shape as /login.
#     """
...
#         else:
#             # create new account — random unusable password since they use Google to sign in
#             import secrets
#             user = models.User(
...
#         # return same shape as /login so frontend handles it identically
#         return {
#             "id": user.id,
#             "name": user.name,
#             "email": user.email,
...
async def fetch_itinerary_weather(itinerary_id: int, user_id: int = Query(...), db: Session = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        for day in itin.days:
            # find a location for the day — from first activity with coordinates or the day's city
            lat, lon, loc_name = None, None, None
            for act in day.activities:
                if act.latitude and act.longitude:
                    lat, lon = act.latitude, act.longitude
                    loc_name = act.location
                    break
            if not lat and day.city:
                loc_name = day.city
                # geocode city name
                try:
                    geo_url = f"http://api.openweathermap.org/geo/1.0/direct?q={day.city},Nepal&limit=1&appid={OPENWEATHER_API_KEY}"
            ...
            try:
                # use forecast API for future dates, current weather for today
                from datetime import date as date_type
                today = date_type.today()

                if day.day_date and day.day_date > today:
                    ...
                    if data.get('list'):
                        ...
                        target = f"{day.day_date} 12:00:00"
