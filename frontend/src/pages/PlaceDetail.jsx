...
const PLACES = {
    bhaktapur: {
        ...
        highlights: [
            { icon: <DirectionsWalkIcon />, label: 'Itinerary', items: [
            ...
            { icon: <RestaurantIcon />, label: 'Food & Drink', items: [
            ...
            { icon: <HotelIcon />, label: 'Where to Stay', items: [
    ...
    kathmandu: {
        ...
        highlights: [
            { icon: <DirectionsWalkIcon />, label: 'Recommended Routes', items: [
            ...
            { icon: <RestaurantIcon />, label: 'Food Guide', items: [
    ...
    pokhara: {
        ...
        highlights: [
            { icon: <DirectionsWalkIcon />, label: '3-Day Itinerary', items: [
            ...
            { icon: <RestaurantIcon />, label: 'Eating Cheap & Well', items: [
            ...
            { icon: <HotelIcon />, label: 'Budget Sleep', items: [
...
const PlaceDetail = () => {
    ...
    return (
        <Box sx={{ bgcolor: COLORS.background, minHeight: '100vh', fontFamily: '"Exo 2", "Segoe UI", sans-serif', transition: 'background-color 0.35s ease' }}>
            <GlobalStyles styles={{
                'html, body, #root': { margin: 0, padding: 0, background: `${COLORS.background} !important` },
                '*': { transition: 'background-color 0.3s ease, border-color 0.3s ease, color 0.3s ease' }
            }} />

            {/* ===== HERO ===== */}
            <Box sx={{ position: 'relative', height: { xs: 320, md: 500 }, overflow: 'hidden' }}>
                <Box component="img" src={place.heroImg} alt={place.title}
                    sx={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                <Box sx={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.65) 100%)' }} />

                {/* Top bar — back + theme toggle */}
                <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: { xs: 2, md: 5 }, py: 2.5 }}>
                    <IconButton onClick={() => navigate(-1)} sx={{ color: 'white', bgcolor: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)', '&:hover': { bgcolor: 'rgba(255,255,255,0.25)' } }}>
                        <ArrowBackIcon />
                    </IconButton>
                    <Tooltip title={isDark ? 'Switch to Light Mode' : 'Switch to Dark Mode'}>
                        <IconButton onClick={toggleTheme} sx={{ color: 'white', bgcolor: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)', '&:hover': { bgcolor: 'rgba(255,255,255,0.25)' } }}>
                ...
                {/* Hero text */}
                <Box sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, px: { xs: 3, md: 6 }, pb: 4 }}>
                    <Typography variant="caption" sx={{ color: COLORS.brand, fontWeight: 700, letterSpacing: 3, textTransform: 'uppercase', fontSize: '0.75rem', mb: 1, display: 'block' }}>
                        {place.location}
                    </Typography>
                    <Typography variant="h3" fontWeight={800} sx={{ color: 'white', fontFamily: '"Exo 2", sans-serif', fontSize: { xs: '1.8rem', md: '2.6rem' }, lineHeight: 1.2, mb: 0.5 }}>
                        {place.title}
                    </Typography>
            ...
            {/* ===== META STRIP ===== */}
            <Box sx={{ bgcolor: COLORS.cardSecondary, borderBottom: `1px solid ${COLORS.borderColor}`, px: { xs: 3, md: 6 }, py: 2, display: 'flex', flexWrap: 'wrap', gap: 3, alignItems: 'center' }}>
                ...
                <Box sx={{ ml: 'auto' }}>
                    <Button variant="contained" onClick={() => navigate('/register')} sx={{ bgcolor: COLORS.brand, color: isDark ? '#141627' : '#FFFFFF', fontWeight: 700, borderRadius: 5, px: 3, textTransform: 'none', '&:hover': { bgcolor: isDark ? '#2db8b8' : '#179898' } }}>
                        Plan This Trip
                    </Button>
                </Box>
            </Box>
            ...
            <Box sx={{ px: { xs: 3, md: 6 }, py: 5, maxWidth: 1200, mx: 'auto' }}>
                ...
                <Stack direction="row" flexWrap="wrap" gap={1} sx={{ mb: 4 }}>
                    {place.tags.map(tag => (
                        <Chip key={tag} label={tag} size="small" sx={{ bgcolor: isDark ? 'rgba(51,204,204,0.12)' : 'rgba(26,175,175,0.1)', color: COLORS.brand, border: `1px solid ${COLORS.borderColor}`, fontWeight: 600, fontSize: '0.75rem' }} />
                    ))}
                </Stack>

                {/* Overview */}
                <Box sx={{ mb: 6 }}>
                    <Typography variant="h5" fontWeight={800} sx={{ color: isDark ? 'white' : COLORS.headings, fontFamily: '"Exo 2", sans-serif', mb: 2 }}>
                        About This Destination
                    </Typography>
                    {place.overview.split('\n\n').map((para, i) => (
                        <Typography key={i} variant="body1" sx={{ color: COLORS.fadedText, lineHeight: 1.9, mb: 2, fontSize: '1rem' }}>{para}</Typography>
                    ))}
                </Box>

                {/* Tabs */}
                <Box sx={{ mb: 5 }}>
                    <Typography variant="h5" fontWeight={800} sx={{ color: isDark ? 'white' : COLORS.headings, fontFamily: '"Exo 2", sans-serif', mb: 3 }}>
                        What to Do & Where to Go
                    </Typography>
                    <Stack direction="row" spacing={1} sx={{ mb: 3, flexWrap: 'wrap', gap: 1 }}>
                        {place.highlights.map((h, i) => (
                            <Button key={i} onClick={() => setActiveTab(i)} sx={{
                                borderRadius: 5, px: 3, py: 1, textTransform: 'none', fontWeight: 600, fontSize: '0.9rem',
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        {place.highlights[activeTab].items.map((item, i) => (
                            <Box key={i} sx={{ bgcolor: COLORS.cardSecondary, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 3, p: 2.5, display: 'flex', gap: 2.5, alignItems: 'flex-start', transition: 'all 0.25s', '&:hover': { borderColor: COLORS.cardBorderHover, transform: 'translateX(4px)', boxShadow: isDark ? '0 4px 20px rgba(51,204,204,0.08)' : '0 4px 20px rgba(26,175,175,0.1)' } }}>
                                <Box sx={{ minWidth: 90, bgcolor: isDark ? 'rgba(51,204,204,0.1)' : 'rgba(26,175,175,0.1)', border: `1px solid ${COLORS.borderColor}`, borderRadius: 2, px: 1.5, py: 0.5, textAlign: 'center' }}>
                                    <Typography variant="caption" fontWeight={700} sx={{ color: COLORS.brand, fontSize: '0.75rem' }}>{item.time}</Typography>
                                </Box>
                                <Typography variant="body2" sx={{ color: COLORS.text, lineHeight: 1.8, flex: 1 }}>{item.detail}</Typography>
                            </Box>
                        ))}
                    </Box>
                </Box>
                ...
                <Box sx={{ mb: 6 }}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2.5 }}>
                        ...
                        <Typography variant="h5" fontWeight={800} sx={{ color: isDark ? 'white' : COLORS.headings, fontFamily: '"Exo 2", sans-serif' }}>Photo Gallery</Typography>
                    </Stack>
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' }, gap: 2 }}>
                        {place.gallery.map((item, i) => (
                            <Box key={i} sx={{ borderRadius: 3, overflow: 'hidden', aspectRatio: '3/2', cursor: 'pointer', position: 'relative', transition: 'transform 0.3s', '&:hover': { transform: 'scale(1.03)' }, '&:hover .caption-overlay': { opacity: 1 }, '&:hover img': { filter: 'brightness(0.75)' } }}>
                                <Box component="img"
                                    src={typeof item === 'string' ? item : item.src}
                                    alt={typeof item === 'string' ? `Photo ${i + 1}` : item.caption}
                                    onError={(e) => { e.target.onerror = null; e.target.src = 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?q=80&w=600&auto=format&fit=crop'; }}
                                    sx={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'filter 0.3s' }} />
                                {typeof item !== 'string' && (
                                    <Box className="caption-overlay" sx={{ position: 'absolute', bottom: 0, left: 0, right: 0, bgcolor: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)', px: 1.5, py: 1, opacity: 0, transition: 'opacity 0.3s ease' }}>
                                        <Typography variant="caption" sx={{ color: 'white', fontWeight: 600, fontSize: '0.72rem', letterSpacing: 0.3 }}>{item.caption}</Typography>
                                    </Box>
                                )}
                            </Box>
                        ))}
                ...
                <Box sx={{ mb: 6 }}>
                    <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 2.5 }}>
                        ...
                        <Typography variant="h5" fontWeight={800} sx={{ color: isDark ? 'white' : COLORS.headings, fontFamily: '"Exo 2", sans-serif' }}>Insider Tips</Typography>
                    </Stack>
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
                        {place.tips.map((tip, i) => (
                            <Box key={i} sx={{ bgcolor: COLORS.cardSecondary, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 3, p: 2.5, display: 'flex', gap: 2, alignItems: 'flex-start', transition: 'all 0.25s', '&:hover': { borderColor: COLORS.cardBorderHover } }}>
                                <Box sx={{ minWidth: 28, height: 28, borderRadius: '50%', bgcolor: COLORS.brand, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, mt: 0.2 }}>
                                    <Typography variant="caption" fontWeight={800} sx={{ color: isDark ? '#141627' : '#FFFFFF', fontSize: '0.75rem' }}>{i + 1}</Typography>
                                </Box>
                                <Typography variant="body2" sx={{ color: COLORS.fadedText, lineHeight: 1.8 }}>{tip}</Typography>
                            </Box>
                        ))}
                    </Box>
                </Box>
                ...
                {/* CTA */}
                <Box sx={{ textAlign: 'center', borderRadius: 5, p: { xs: 4, md: 6 }, border: `1px solid ${COLORS.borderColor}`, background: isDark ? `linear-gradient(135deg, #252845, rgba(51,204,204,0.05))` : `linear-gradient(135deg, #FFFFFF, rgba(26,175,175,0.05))`, boxShadow: isDark ? 'none' : '0 4px 24px rgba(0,0,0,0.07)' }}>
                    <Typography variant="h5" fontWeight={800} sx={{ color: isDark ? 'white' : COLORS.headings, fontFamily: '"Exo 2", sans-serif', mb: 1.5 }}>
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="center">
                        ...
                        <Button variant="outlined" onClick={() => navigate(-1)} sx={{ color: isDark ? 'white' : COLORS.headings, borderColor: isDark ? 'rgba(255,255,255,0.3)' : 'rgba(26,175,175,0.4)', borderRadius: 5, px: 4, py: 1.5, fontSize: '1rem', textTransform: 'none', '&:hover': { borderColor: COLORS.brand, color: COLORS.brand, bgcolor: 'transparent' } }}>
                            ← Back to Plans
                        </Button>
                    </Stack>
                </Box>
            ...
            {/* Footer */}
            <Box sx={{ borderTop: `1px solid ${COLORS.borderColor}`, py: 3, px: { xs: 3, md: 6 }, textAlign: 'center', bgcolor: COLORS.cardSecondary, mt: 6 }}>
