...
import {
    ...
    CircularProgress, InputAdornment, IconButton, Stack, GlobalStyles
} from '@mui/material';
    ...
    return (
        <Box sx={{ display: 'flex', height: '100vh', bgcolor: COLORS.background, overflow: 'hidden' }}>
            ...
            <Box sx={{ display: { xs: 'none', md: 'block' }, width: '50%', flexShrink: 0, position: 'relative', overflow: 'hidden', alignSelf: 'stretch' }}>
                <Box component="img" src="https://images.unsplash.com/photo-1660990983694-b0a200676520?w=900&q=85" alt="Nepal prayer flags"
                    sx={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', display: 'block' }} />
            </Box>

            {/* Right — Form */}
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', px: { xs: 4, md: 7 }, py: 4, overflowY: 'auto' }}>
                <Box sx={{ maxWidth: 360, width: '100%', mx: 'auto' }}>
                    <Typography variant="h4" fontWeight={800} sx={{ color: COLORS.headings, mb: 0.75 }}>Create Your Account</Typography>
                    ...
                    <Box component="form" onSubmit={handleSubmit}>
                        <Stack spacing={1.5}>
                            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5 }}>
                                <TextField required fullWidth name="firstName" placeholder="First Name" autoComplete="given-name" autoFocus
                                    value={formData.firstName} onChange={handleChange}
                                    error={Boolean(validationErrors.firstName)} helperText={validationErrors.firstName}
                                    disabled={loading} sx={inputSx}
                                    InputProps={{ startAdornment: <InputAdornment position="start"><PersonIcon /></InputAdornment> }} />
                                <TextField required fullWidth name="lastName" placeholder="Last Name" autoComplete="family-name"
                                    value={formData.lastName} onChange={handleChange}
                                    error={Boolean(validationErrors.lastName)} helperText={validationErrors.lastName}
                                    disabled={loading} sx={inputSx} />
                            </Box>

                            <TextField required fullWidth name="email" placeholder="Email Address" autoComplete="email" type="email"
                                value={formData.email} onChange={handleChange}
                                error={Boolean(validationErrors.email)} helperText={validationErrors.email}
                                disabled={loading} sx={inputSx}
                                InputProps={{ startAdornment: <InputAdornment position="start"><EmailIcon /></InputAdornment> }} />

                            <TextField required fullWidth name="password" placeholder="Enter Your Password" type={showPassword ? 'text' : 'password'} autoComplete="new-password"
                                value={formData.password} onChange={handleChange}
                                error={Boolean(validationErrors.password)} helperText={validationErrors.password}
                                disabled={loading} sx={inputSx}
                                InputProps={{
                                    startAdornment: <InputAdornment position="start"><LockIcon /></InputAdornment>,
                                    endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowPassword(s => !s)} disabled={loading} sx={{ color: COLORS.fadedText, p: 0.5, '&:hover': { color: COLORS.brand } }}>{showPassword ? <VisibilityOff sx={{ fontSize: 18 }} /> : <Visibility sx={{ fontSize: 18 }} />}</IconButton></InputAdornment>,
                                }} />

                            <TextField required fullWidth name="confirmPassword" placeholder="Confirm Password" type={showConfirmPassword ? 'text' : 'password'} autoComplete="new-password"
                                value={formData.confirmPassword} onChange={handleChange}
                                error={Boolean(validationErrors.confirmPassword)} helperText={validationErrors.confirmPassword}
                                disabled={loading} sx={inputSx}
                                InputProps={{
                                    startAdornment: <InputAdornment position="start"><LockIcon /></InputAdornment>,
                                    endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowConfirmPassword(s => !s)} disabled={loading} sx={{ color: COLORS.fadedText, p: 0.5, '&:hover': { color: COLORS.brand } }}>{showConfirmPassword ? <VisibilityOff sx={{ fontSize: 18 }} /> : <Visibility sx={{ fontSize: 18 }} />}</IconButton></InputAdornment>,
                                }} />

                            <Button type="submit" fullWidth variant="contained" disabled={loading} sx={{ py: 1.4, mt: 0.5, bgcolor: COLORS.brand, color: COLORS.background, fontWeight: 700, fontSize: '0.95rem', borderRadius: 2, textTransform: 'none', boxShadow: 'none', letterSpacing: 0.3, transition: 'all 0.25s', '&:hover': { bgcolor: '#2db8b8', boxShadow: '0 4px 16px rgba(51,204,204,0.35)', transform: 'translateY(-1px)' }, '&:disabled': { bgcolor: 'rgba(51,204,204,0.25)', color: COLORS.fadedText } }}>
                        ...
                        <Box sx={{ textAlign: 'center', mt: 3 }}>
                            <Typography variant="body2" sx={{ color: COLORS.fadedText, fontSize: '0.85rem' }}>
                                ...
                                <Link component="button" type="button" onClick={() => navigate('/login')} disabled={loading}
